            ...
            <v-card width="100%">
              <v-card-text>
                <v-layout align-baseline>
                  ...
                  <v-flex md2>
                    <v-checkbox hide-details label="Save" v-model="isSaveFrom" class="checkbox"></v-checkbox>
                  </v-flex>
                  <v-flex md4>
                    <v-text-field
                      hide-details
              ...
              <v-flex xs6 md2>
                <v-text-field 
                  ...
                  :rules="[
                    ...
                    fieldIsOverZero('quantity'),
                    limitMultipleShipping
                  ]"
                  class="required_field"></v-text-field>
              </v-flex>
              <v-flex v-if="shippingService === 'USPS'" xs6 md2 text-xs-center style="border: 1px solid black; border-radius: 2px;">
                USPS label cannot be canceled
              </v-flex>
              <v-flex xs6 :md7="shippingService === 'USPS'" :md8="shippingService !== 'USPS'">
                        ...
                        <v-card-text>
                          ...
                          >
                            <template v-slot:items="{ item }">
                              <td ><a linkify @click="() => {
                                $refs['LabelPackageWidget'].shippingService = item.serviceType === 'GROUND_HOME_DELIVERY' ? 'FEDEX_GROUND' : item.serviceType
...
import LabelPackageWidget from './LabelPackageWidget'
...
export default {
  ...
  mounted () {
    this.from = this.initFrom ? cloneDeep(this.initFrom) : {}
    this.to = this.initTo ? cloneDeep(this.initTo) : {}
  },
  watch: {
    toTemplate (value) {
  ...
  methods: {
    ...
    extractPayload () {
      ...
      const {
        ...
        memo,
        note
      } = this.$refs['LabelPackageWidget'].$$extractData()
    ...
    onSubmitted () {
      ...
      let {isSaveFrom, isSaveTo, quantity, to, from} = this
      ...
      const labels = (this.requestIdCache.length ? this.requestIdCache : this.idGenerator(quantity))
        .map(requestId => {
          return {
            ...payload,
            isMeasurementMetric: this.isMeasurementMetric,
      ...
      return this.$store.dispatch('createLabels', labels)
        .then(rtn => {
          this.requestIdCache = []
        ...
        .catch(error => {
          if (error.message === 'request-label-error') {
            ...
            this.errorMessage = 'Endpoint request timed out. Please wait a few minutes and retry.'
            this.requestIdCache = labels.map(item => item.requestId)
          } else {
