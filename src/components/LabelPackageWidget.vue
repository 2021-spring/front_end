  ...
  <div>
    ...
    <v-layout justify-space-between>
      <v-flex md7>
        <v-text-field
          label="Note"
          hint="Note will not be printed on label"
          v-model.trim="note"
      ...
      <v-flex md4>
        <v-text-field
          label="Memo"
          hint="Memo will be printed on label"
      ...
      <template v-slot:items="props">
        ...
        <td class="text-xs-left">
          <v-text-field
            ...
            @change="(e) => defaultZero(e, props.item)"
        ...
        <td class="text-xs-left">
          <v-text-field
            class="table-item"
            hide-details
            v-model="props.item.memo"></v-text-field>
        </td>
      </template>
    </v-data-table>
  </div>
...
export default {
  ...
  computed: {
    headers () {
      return [
        ...
        { text: 'Memo', value: 'memo', align: 'left', sortable: false, width: '25%' }
      ]
    },
    isInsuranceEnabled () {
  ...
  methods: {
    ...
    checkEnableMWT (shippingService) {
      for (let labelService of this.$store.getters.labelServices) {
