...
(function() {
  var CARRIERS, uniq, _checkDigit, _confirmA1International, _confirmCanadaPost16, _confirmFedex12, _confirmFedex15, _confirmFedex20, _confirmFedex30, _confirmFedex34, _confirmFedex9222, _confirmFedex9622, _confirmFedexDoorTag, _confirmFedexSmartPost, _confirmUps, _confirmUpsFreight, _confirmUsps20, _confirmUsps22, _confirmUsps26, _confirmUsps420Zip, _confirmUsps420ZipPlus4, _preprocess;
  ...
  _confirmFedex30 = function(trk) {
    var alteredTrk = trk.slice(8);
    if (_checkDigit(alteredTrk, [3, 1], 10)) {
      return [true, false, alteredTrk.slice(2), alteredTrk];
    }
    return [false, false];
  };

  _confirmFedex34 = function(trk) {
    if (trk.slice(20,22) === '00' && _checkDigit(trk.slice(22), [3, 1, 7], 11)) {
      return [true, true, trk.slice(22)];
    }
    return [false, false];
  };
  ...
  _confirmFedex9222 = function(trk) {
    if (_checkDigit(trk, [3, 1], 10)) {
      return [true, false, trk.slice(2), trk];
    }
    return [false, false];
  };
  ...
  _confirmUsps420ZipPlus4 = function(trk) {
    if (_checkDigit(trk.match(/^420\d{9}(\d{22})$/)[1], [3, 1], 10)) {
      return [true, false, trk.slice(12)];
    } else {
  ...
  CARRIERS = [
    {
      ...
      name: 'fedex',
      regex: /^420\d{5}92\d{20}$/,
      confirm: _confirmFedex30
    }, {
      name: 'usps',
      regex: /^420\d{27}$/,
  ...
  module.exports = function(trk) {
    ...
    CARRIERS.every(function(c) {
      ...
      if (trk.match(c.regex)) {
        if (c.confirm != null) {
          _ref = c.confirm(trk), good = _ref[0], stop = _ref[1], tracking = _ref.slice(2);
          if (tracking.length === 0) {
            tracking = [trk]
          }
          if (good) {
            carriers.push({name: c.name, tracking: tracking });
          }
          return !stop;
        }
        carriers.push({name: c.name, tracking: [trk]});
        return true;
      }
      return true;
    });
    let result = carriers.reduce( (accu, carrier) => {
      if (accu.every( item => item.name !== carrier.name)) accu.push(carrier);
      return accu;
    }, []);
    return result;
  };
